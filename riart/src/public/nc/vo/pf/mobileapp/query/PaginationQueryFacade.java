...
public class PaginationQueryFacade {
	...
	public List<Map<String, Object>> query(TaskQuery query,int startline,int count,String condition,
		String[] pkOrg,String[] billmaker,String[] moudles,String statuskey,String datetype) throws BusinessException {
		String[] realPks = queryPks(query, condition,statuskey,pkOrg,billmaker,moudles,startline,count,datetype);
		if(realPks==null){
			realPks=new String[]{};
		}
		return query.queryByPks(realPks);
	 ...
	 * @throws BusinessException
	 */
   private String[] queryPks(TaskQuery query, String condition,String statuskey,String[] pkOrg,String[] billmaker,String[] moudles,int startline,int count,String datetype) throws BusinessException{
	   ...
	   /**先过滤，再进行分页*/
		sql = this.filterBillTypeSql(query,sql,condition);
		sql=FilterConditionUtil.queryPkByOrgAndBillmaker(statuskey,sql, pkOrg, billmaker,moudles,datetype);
		buffer.append(sql);
		/**添加排序*/
...
private String[] queryPks(TaskQuery query, String condition,String statuskey,String[] pkOrg,String[] billmaker,String[] moudles,int startline,int count) throws BusinessException{
	   ...
	   /**先过滤，再进行分页*/
		sql = this.filterBillTypeSql(query,sql,condition);
		sql=FilterConditionUtil.queryPkByOrgAndBillmaker(statuskey,sql, pkOrg, billmaker,moudles);
		buffer.append(sql);
		/**添加排序*/
		buffer.append(querySql);
		LimitSQLBuilder builder = SQLBuilderFactory.getInstance()
   ...
   /**
	 ...
	 * @throws BusinessException
	 */
   public Integer query(TaskQuery query,String condition,String[] pkOrg,String[] billmaker,String[] moudles,String statuskey,String datetype)
			throws BusinessException {
		
		return queryPks(query, condition,pkOrg,billmaker,moudles,statuskey,datetype);
	}
   ...
   private Integer queryPks(TaskQuery query, String condition,String[] pkOrg,String[] billmaker,String[] moudles,String statuskey,String datetype) throws BusinessException {
		...
		sql = sql.substring(0, strat);
		sql = this.filterBillTypeSql(query,sql,condition);
		sql=FilterConditionUtil.countqueryPkByOrgAndBillmaker(statuskey,sql, pkOrg, billmaker,moudles,datetype);
		String[] pks = queryPks(sql);
		return pks.length;
