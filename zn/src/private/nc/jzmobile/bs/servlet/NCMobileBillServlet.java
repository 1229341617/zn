...
public class NCMobileBillServlet extends HttpServlet implements
		IHttpServletAdaptor {
	 ...
	 */
	static {
		map.put("getDataList", "GetDataListHandler");
		map.put("getTemplet", "GetBillTempletHandler");
		map.put("getDefaultList", "GetDefaultListDataHandler");
		map.put("getBillData", "GetBillDataHandler");
		map.put("getRefData", "GetRefHandler");
		map.put("billSave", "BillSaveHandler");
		//内网 请假单 提交
		map.put("JZMNGBillSaveAndSubmit", "JZMNGBillSaveAndSubmitHandler");
		//内网 请假单 请假类型数据字典
		map.put("JZMNGHolidayTypeList", "JZMNGHolidayTypeListHandler");
		map.put("billSubmit", "BillSubmitHandler");
		map.put("carryValueByRef", "CarryValueByRefHandler");
		// * 同步 收入类、支出类 合同 
		map.put("SyncJZCTContract", "SyncJZCTContractHandler");
		
		/**获取NC默认设置并进行缓存*/
		map.put("defaultSetting", "GetDefaultSettingHandler");
		/**删除单据*/
		map.put("delete", "DeleteBillHandler");
		
		/**附件列表*/
		map.put("fileList", "FileListHandler");
		
		/**查看附件*/
		map.put("fileContent", "FileContentHandler");
		
		/**收回单据*/
		map.put("billRecall", "RecallBillHandler");
		/**上传附件*/
		map.put("uploadFile", "UploadFileHandler");
		/**删除附件*/
		map.put("deleteFile", "DeleteFileHandler");
		
		
		/**移动收料代码*/
		map.put("mrBillSave", "MRBillSaveHandler");
		map.put("mrGetTemplet", "MRGetBillTempletHandler");
		map.put("mrGetDefaultList", "MRGetDefaultListDataHandler");
		map.put("mrGetRefData", "MRGetRefHandler");
	...
	public void doAction(HttpServletRequest req, HttpServletResponse resp)
		...
		Logger.error("==========NCMobileBillServlet start==========");
		...
		InvocationInfoProxy.getInstance().setUserDataSource(
				new MobilePropertiesLoader().getViewResolverProperties()
						.getDataSource());
		Logger.error("==========NCMobileBillServlet InvocationInfoProxy1==========");
		...
		try {
			Logger.error("==========NCMobileBillServlet InvocationInfoProxy  try==========");
			...
			Logger.error("==========NCMobileBillServlet InvocationInfoProxy  handler==========");
		...
		out.flush();
		out.close();
		Logger.error("==========NCMobileBillServlet end==========");
